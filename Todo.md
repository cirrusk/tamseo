...
- 상태: DONE
- 목표:
  - 메타 설명문을 네이버 권장 기준(80자 이내)에 맞게 축약
- 작업 항목:
  1. DONE: `src/app/layout.tsx` `description` 문구 축약
  2. DONE: 빌드 검증 및 결과 기록
- 결과:
  - `description`을 46자 문구로 축약(80자 이내 충족)
  - 새 문구: `서울시 도서관 책을 한 번에 검색해, 원하는 책이 많은 가까운 도서관을 찾는 탐서.`
  - `npm run build` 성공
